...
function getPayDunyaApiBase() {
  return process.env.PAYDUNYA_MODE === 'sandbox'
    ? 'https://app.paydunya.com/sandbox-api/v1'
    : 'https://app.paydunya.com/api/v1';
}

function getPayDunyaCallbackUrl() {
...
async function createPayDunyaFallbackLink({ amount, orderId, phone, customData = {}, paymentMethod = 'wave' }) {
  ...
  const payload = {
    invoice: {
      ...
      custom_data: {
        ...
        fallback_reason: 'pixpay_missing_sms_link'
      }
    },
    store: {
      name: storeName
    ...
    actions: {
      cancel_url: process.env.PAYDUNYA_CANCEL_URL || `${basePublicUrl}/payment-error`,
      return_url: returnUrl,
      callback_url: getPayDunyaCallbackUrl()
    }
  };

  console.warn('[PAYDUNYA-FALLBACK] PixPay sans sms_link, creation facture checkout:', {
    orderId,
    amount: amountNumber,
    paymentMethod
  });
  ...
  const response = await axios.post(
    `${getPayDunyaApiBase()}/checkout-invoice/create`,
    payload,
    {
      headers: {
  ...
  const checkoutLink = invoiceData.response_text || invoiceData.url || invoiceData.redirect_url;

  if (invoiceData.response_code !== '00' || !invoiceToken) {
    throw new Error(invoiceData.response_text || invoiceData.description || 'Fallback PayDunya indisponible');
  }

  // Lier la commande au token PayDunya pour le webhook de confirmation.
  try {
    const dbClient = (SUPABASE_SERVICE_ROLE_KEY && SUPABASE_URL)
    ...
    if (dbClient) {
      ...
      if (orderUpdateError) {
        console.warn('[PAYDUNYA-FALLBACK] Impossible de stocker le token commande:', orderUpdateError);
      }
    }
  } catch (orderUpdateErr) {
    console.warn('[PAYDUNYA-FALLBACK] Erreur update token commande:', orderUpdateErr?.message || orderUpdateErr);
  }

  // Important: for Wave fallback we must use SoftPay (no checkout redirect page).
  if (paymentMethod === 'wave') {
    const wavePhone = formatPhoneForPayDunyaWave(phone);
    if (!wavePhone) {
      throw new Error('Numéro Wave manquant pour le fallback SoftPay');
    }

    const waveEmail = safeCustomData.email
      ? String(safeCustomData.email)
      : (process.env.PAYDUNYA_WAVE_FALLBACK_EMAIL || 'client@validele.app');
    const waveFullName = safeCustomData.fullName ? String(safeCustomData.fullName) : 'Client Validele';

    console.warn('[PAYDUNYA-FALLBACK] Wave: tentative SoftPay direct (sans checkout PayDunya)', {
      orderId,
      phone: wavePhone
    });

    let softpayResponse;
    if (process.env.PAYDUNYA_MODE === 'sandbox') {
      softpayResponse = await axios.post(
        'https://app.paydunya.com/sandbox-api/v1/checkout/make-payment',
        {
          phone_number: wavePhone,
          customer_email: waveEmail,
      ...
      softpayResponse = await axios.post(
        `${getPayDunyaApiBase()}/softpay/wave-senegal`,
        {
          wave_senegal_fullName: waveFullName,
          wave_senegal_email: waveEmail,
          wave_senegal_phone: wavePhone,
    ...
    const softpayData = softpayResponse?.data || {};
    const softpayLink =
      softpayData.url ||
    ...
    if (!softpayLink) {
      throw new Error('SoftPay Wave n\'a pas renvoyé de lien de paiement');
    }
    ...
    return {
      ...
      state: 'PENDING',
      message: 'PixPay indisponible, bascule Wave vers SoftPay PayDunya.',
      sms_link: softpayLink,
      amount: amountNumber,
      fee: null,
      raw: {
        paydunya: {
  ...
  if (!checkoutLink) {
    throw new Error('Fallback checkout PayDunya indisponible: lien manquant');
  }
  ...
  try {
    ...
    if (result?.success && !result?.sms_link) {
      ...
      try {
        ...
        console.error('[PAYDUNYA-FALLBACK] Echec fallback Orange:', fallbackError?.response?.data || fallbackError?.message || fallbackError);
      }
    }
    ...
    if (finalResult.success && finalResult.transaction_id) {
      const { error: dbError } = await supabase
        .from('payment_transactions')
        .insert({
          transaction_id: finalResult.transaction_id,
          provider,
          provider_transaction_id: finalResult.provider_id,
          order_id: orderId,
          amount,
          phone,
          status: finalResult.state || 'PENDING1',
          raw_response: normalizeJsonField(finalResult.raw)
        });
    ...
    return res.json({
      ...
      transaction_id: finalResult.transaction_id,
      provider_id: finalResult.provider_id,
      provider,
      fallback_used: fallbackUsed,
      fallback_reason: fallbackReason,
      message: finalResult.message,
      sms_link: finalResult.sms_link,
  ...
  try {
    ...
    if (!result?.success || !result?.sms_link) {
      ...
      try {
        ...
        console.error('[PAYDUNYA-FALLBACK] Echec fallback Wave:', fallbackError?.response?.data || fallbackError?.message || fallbackError);
      }
    }
...
async function notifyBuyerWhatsAppPaymentConfirmed(orderId) {
  try {
    ...
    try {
      ...
      console.warn('[WHATSAPP] CTA paiement refusé, repli template statique:', ctaError?.message || ctaError);
      ...
      try {
        ...
        if (orderId) {
          ...
          if (orderExists && orderExists.id) {
            ...
            // Correction: update payout_batch_items by provider_transaction_id (for both Orange and Wave)
            // 1. Update all items in the batch with this provider_transaction_id to 'paid'
            ...
            if (itemsToUpdate && itemsToUpdate.length > 0) {
              ...
              const orderIds = itemsToUpdate.map(i => i.order_id).filter(Boolean);
            ...
            // 2. Vérifie si tous les items du batch sont maintenant 'paid' et complète le batch si oui
...
app.post('/api/admin/refund-requests/:id/approve', requireAdmin, async (req, res) => {
  try {
    ...
    // 6bis) Notifier l'acheteur que le remboursement est effectué : procédure
    // habituelle (WhatsApp d'abord, SMS de secours si non lu après 10 min), comme
    // pour la livraison. Non bloquant : un échec n'empêche pas le remboursement.
    if (result.success && buyerPhone) {
      try {
        const { notifyRefundProcessedWithFallback } = require('./whatsapp-bot');
        const walletLabel = walletType === 'wave-senegal' ? 'Wave'
          : walletType === 'orange-senegal' ? 'Orange Money'
          : 'votre compte mobile';
        await notifyRefundProcessedWithFallback({
          orderId: refundRequest.order_id,
          buyerPhone,
          amount: refundRequest.amount,
          walletLabel,
        });
        console.log('[REFUND] Notification remboursement (WhatsApp + fallback SMS) programmée pour', buyerPhone);
      } catch (notifErr) {
        console.warn('[REFUND] Echec notification remboursement (non bloquant):', notifErr && notifErr.message);
      }
    }
