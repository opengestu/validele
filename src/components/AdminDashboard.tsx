        ...
        {activeTab === 'orders' && (
          <Card>
            ...
            <CardContent>
              ...
              {/* Section: Commandes en attente de paiement (pending) */}
              <div className="mt-8">
                <h3 className="text-lg font-semibold mb-4 text-orange-700">📋 Commandes en attente de paiement</h3>
                <Table>
                  <TableHeader>
                    <TableRow>
                      <TableHead className="w-20">ID</TableHead>
                  ...
                  <TableBody>
                    {filterOrders(orders)
                      ...
                      .map((o: OrderFull) => (
                        <TableRow key={o.id}>
                          <TableCell className="font-mono text-xs truncate max-w-[80px]" title={o.id}>{o.id.substring(0, 8)}...</TableCell>
                          <TableCell className="font-semibold">{o.order_code}</TableCell>
                          <TableCell>{o.buyer?.full_name || '-'}</TableCell>
                          <TableCell>{o.buyer?.phone || '-'}</TableCell>
                          <TableCell className="font-semibold">{o.total_amount?.toLocaleString()} FCFA</TableCell>
                          <TableCell>
                          ...
                          <TableCell className="flex gap-2">
                            <Button 
                              ...
                              disabled={processing}
                              onClick={async () => {
                                if (!confirm(`Confirmer le paiement de la commande ${o.order_code} (${o.total_amount?.toLocaleString()} FCFA) ?\n\nCette action marquera la commande comme payée.`)) return;
                                setProcessing(true);
                                try {
                                  const res = await fetch(apiUrl('/api/admin/confirm-payment'), {
                                    method: 'POST',
                                  ...
                                  const json = await res.json();
                                  if (!res.ok) throw new Error(json?.error || 'Erreur confirmation paiement');
                                  toast({ title: 'Paiement confirmé ✓', description: `Commande ${o.order_code} marquée comme payée` });
                                  fetchData();
                                } catch (err: unknown) {
                            ...
                            >
                              {processing ? '⏳...' : '✓ Confirmer Paiement'}
                            </Button>
                          </TableCell>
                        </TableRow>
                      ))}

                    {filterOrders(orders).filter(o => String(o.status).toLowerCase() === 'pending').length === 0 && (
                      <TableRow>
                        <TableCell colSpan={8} className="text-center text-gray-500">Aucune commande en attente</TableCell>
                      </TableRow>
                    )}
                  </TableBody>
                </Table>
